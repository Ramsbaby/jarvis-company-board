...
const MonacoEditor = dynamic(() => import('@monaco-editor/react'), { ssr: false, loading: () => (
  <div className="flex-1 flex items-center justify-center bg-zinc-900 rounded-xl text-zinc-500 text-sm">에디터 로딩 중...</div>
)});
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex flex-col gap-1 items-center">
        ...
        {isUrgent && (
          <span className="text-[9px] text-red-400 font-bold animate-pulse">⚡ 마감 임박</span>
        )}
      </div>
    </div>
  );
...
export default function LiveCodingClient({
    ...
    return (
      <div className="fixed inset-0 z-50 bg-zinc-950 flex flex-col">
        <div className="flex items-center justify-between px-4 py-2 bg-zinc-900 border-b border-zinc-700">
          <div className="flex items-center gap-3">
            <span className="text-xs font-bold text-zinc-400">☕ Java — {problem.title}</span>
          ...
          <div className="flex items-center gap-2">
            {timerRunning && <Timer running={timerRunning} onExpire={handleExpire} />}
  ...
  return (
    <div className="bg-zinc-950 min-h-screen flex flex-col text-zinc-100">
      {/* Header — 다크 테마 */}
      <header className="sticky top-0 z-20 bg-zinc-900 border-b border-zinc-700/60 shadow-lg shadow-zinc-950/50">
        <div className="max-w-[1400px] mx-auto px-4 py-2.5 flex items-center gap-3">
          <Link href="/interview" className="text-zinc-500 hover:text-zinc-300 text-xs shrink-0 transition-colors">← 면접 홈</Link>
      ...
      {!started && (
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="bg-zinc-900 rounded-2xl border border-zinc-700 p-8 max-w-md w-full text-center space-y-6 shadow-2xl shadow-zinc-950">
            ...
            <div className="bg-zinc-800/60 rounded-xl p-4 text-left space-y-2 border border-zinc-700">
              ...
              <ul className="text-xs text-zinc-400 space-y-1.5">
                <li className="flex items-center gap-2"><span className="text-zinc-500">⏱</span> 제한 시간 <strong className="text-zinc-200">30분</strong></li>
                <li className="flex items-center gap-2"><span className="text-zinc-500">☕</span> 언어 <strong className="text-zinc-200">Java</strong></li>
                <li className="flex items-center gap-2"><span className="text-zinc-500">🔍</span> 웹 검색 · AI 도구 <strong className="text-zinc-200">허용</strong></li>
                <li className="flex items-center gap-2"><span className="text-zinc-500">💡</span> 힌트 1회 가능 <span className="text-zinc-500">(감점 없음)</span></li>
              </ul>
            </div>
            <div className="bg-indigo-950/60 border border-indigo-800/60 rounded-xl p-3.5">
              <p className="text-xs text-indigo-300">
                <span className="text-indigo-500 font-bold">문제 </span>{problem.title}
              </p>
            </div>
            <button
              onClick={handleStart}
      ...
      {started && (
        <div
          ref={containerRef}
          className="flex-1 flex max-w-[1400px] mx-auto w-full px-3 py-3 gap-0"
          style={{ minHeight: 'calc(100vh - 52px)' }}
        >
          {/* 왼쪽: 문제 패널 */}
          <div
            className="flex flex-col gap-2 pr-2 overflow-hidden"
            style={{ width: `${leftPct}%`, minWidth: '280px' }}
          >
            {/* 탭 */}
            ...
            {/* 문제 내용 */}
            <div className="bg-zinc-900 rounded-xl border border-zinc-700 p-4 flex-1 overflow-y-auto">
              {activeTab === 'problem' && (
                ...
                <div className="space-y-3">
                  {problem.examples.map((ex, i) => (
                    <div key={i} className="space-y-2">
                      ...
                      <div className="bg-zinc-800 rounded-lg p-3 space-y-1 border border-zinc-700">
              ...
              <div className="bg-zinc-900 rounded-xl border border-zinc-700 p-3">
                {!hint ? (
                  <button
                    ...
                    className="w-full text-xs text-amber-400 hover:text-amber-300 font-semibold flex items-center justify-center gap-1.5 py-1 disabled:opacity-50 transition-colors"
                  >
          ...
          {/* 오른쪽: 에디터 + 피드백 */}
          <div className="flex flex-col gap-2 pl-1 overflow-hidden flex-1">
            {/* 만료 배너 */}
            {expired && !feedback && (
              <div className="bg-red-950/80 border border-red-800 rounded-xl p-2.5 flex items-center gap-2">
              ...
              <div className="flex flex-col flex-1 gap-2">
                ...
                <div className="flex items-center justify-between px-1">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-0.5 ml-1">
                      <button
                        ...
                        className="w-5 h-5 rounded bg-zinc-800 text-zinc-400 text-xs flex items-center justify-center hover:bg-zinc-700 hover:text-zinc-200 transition-colors border border-zinc-700"
                        title="글자 축소"
                      >−</button>
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      onClick={handleCopy}
                      className="text-[10px] px-2 py-1 rounded-lg bg-zinc-800 text-zinc-400 hover:text-zinc-200 border border-zinc-700 transition-colors"
                    >
                      {copied ? '✓ 복사됨' : '📋 복사'}
                    </button>
                    <button
                    ...
                    <button
                      ...
                      className="text-[10px] px-2 py-1 rounded-lg bg-zinc-800 text-zinc-400 hover:text-red-400 border border-zinc-700 transition-colors"
                    >
                      ↺ 초기화
                    </button>
                  </div>
                ...
                {/* Monaco 에디터 */}
                <div className="flex-1 rounded-xl overflow-hidden border border-zinc-700 shadow-lg" style={{ minHeight: '400px' }}>
                  <MonacoEditor
                    height="100%"
                    language="java"
                    theme="vs-dark"
              ...
              <div className="space-y-3 overflow-y-auto flex-1">
                ...
                {feedback.goodPoints.length > 0 && (
                  <div className="bg-emerald-950/50 rounded-xl border border-emerald-800/60 p-4 space-y-2">
                    ...
                    {feedback.goodPoints.map((p, i) => (
                      <p key={i} className="text-xs text-emerald-300 flex gap-2"><span className="text-emerald-600 shrink-0">•</span>{p}</p>
                    ))}
                  </div>
                )}
                ...
                {feedback.improvements.length > 0 && (
                  <div className="bg-amber-950/50 rounded-xl border border-amber-800/60 p-4 space-y-2">
                    ...
                    {feedback.improvements.map((p, i) => (
                      <p key={i} className="text-xs text-amber-300 flex gap-2"><span className="text-amber-600 shrink-0">•</span>{p}</p>
                    ))}
                  </div>
                )}
                ...
                {feedback.edgeCases.length > 0 && (
                  <div className="bg-red-950/50 rounded-xl border border-red-800/60 p-4 space-y-2">
                    ...
                    {feedback.edgeCases.map((p, i) => (
                      <p key={i} className="text-xs text-red-300 flex gap-2"><span className="text-red-600 shrink-0">•</span>{p}</p>
                    ))}
                  </div>
                )}
                ...
                <div className="flex gap-2 pb-4">
                  <Link
                    href="/interview"
                    className="flex-1 py-3 rounded-xl border border-zinc-700 bg-zinc-800 text-zinc-300 font-semibold text-xs text-center hover:bg-zinc-700 transition-colors"
                  >
                    ← 면접 홈
                  </Link>
                  <a
