...
export default function AutoPostToggle({ initialPaused }: { initialPaused: boolean }) {
  ...
  return (
    <button
      ...
      className={`flex items-center gap-1 px-2 py-1 rounded-md border text-xs font-medium transition-colors disabled:opacity-50 whitespace-nowrap ${
        paused
          ? 'bg-amber-50 border-amber-300 text-amber-700 hover:bg-amber-100'
          : 'border-zinc-200 text-zinc-500 hover:bg-zinc-50'
      }`}
    >
      {paused ? '⏸' : '▶'}<span className="hidden sm:inline">{paused ? ' 게시 정지 중' : ' 자동 게시'}</span>
    </button>
  );
}
