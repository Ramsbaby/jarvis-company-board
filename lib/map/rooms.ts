...
export const ROOMS: RoomDef[] = [
  ...
  // 구 학습팀 자리 → 라이브러리 (신설). 기록팀이 관리하는 데이터를 검색/탐색하는 프론트엔드.
  { id: 'library',     entityId: 'library',      name: '라이브러리', emoji: '📖', description: '전사 지식 베이스 프론트엔드. 기록팀이 관리하는 RAG 인덱스(12만+ 청크)와 오너 메모리(~/.claude/.../memory) 검색·탐색 허브. 기록팀=백엔드, 라이브러리=사용자 접근.',                         x: 20, y: 10, w: 7, h: 5, type: 'team',    npcX: 23, npcY: 12, teamColor: '#0ea5e9', floorStyle: 'carpet' },
  ...
  // Row 3 (y=18)
  // 구 커리어팀 자리 → 성장실. 학습 + 커리어 통합 (면접 준비 + 기술 공부 현실 반영)
